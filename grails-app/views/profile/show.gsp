	...
	<body>
		...
		<div id="show-profile" class="content scaffold-show" role="main">
			...
			<ol class="property-list profile">
				...
				<g:if test="${profileInstance?.conversations}">
				<li class="fieldcontain">
					<span id="conversations-label" class="property-label"><g:message code="profile.conversations.label" default="Conversations" /></span>
					
						<g:each in="${profileInstance.conversations}" var="c">
						<span class="property-value" aria-labelledby="conversations-label"><g:link controller="conversation" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
