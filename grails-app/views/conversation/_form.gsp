...
<div class="fieldcontain ${hasErrors(bean: conversationInstance, field: 'owner', 'error')} ">
	...
	<g:select id="owner" name="owner.id" from="${org.code4good.User.list()}" optionKey="id" value="${conversationInstance?.owner?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: conversationInstance, field: 'profile', 'error')} required">
	...
	<g:select id="profile" name="profile.id" from="${org.code4good.Profile.list()}" optionKey="id" required="" value="${conversationInstance?.profile?.id}" class="many-to-one"/>
